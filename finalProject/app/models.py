...
class Product(db.Model):
    ...
    author = db.Column(db.String(64))  # Добавляем поле автора как строку
...
class Order(db.Model):
    ...
    status = db.Column(db.String(50), nullable=False, default='Ожидает звонка/сообщения')
...
class Review(db.Model):
    ...
    dislikes = db.relationship('Dislike', backref='review', lazy='dynamic')
...
class SavedProduct(db.Model):
    ...
    product = db.relationship('Product', back_populates='saved_by')


User.saved_products = db.relationship('SavedProduct', order_by=SavedProduct.timestamp, back_populates='user')
Product.saved_by = db.relationship('SavedProduct', order_by=SavedProduct.timestamp, back_populates='product')
